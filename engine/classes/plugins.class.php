...
if ( !$config['version_id'] ) {
	...
	if ( file_exists(ROOT_DIR . '/install.php') AND !file_exists(ENGINE_DIR . '/data/config.php') ) {
		...
		die ( "not installed. Please run install.php" );
...
if( version_compare(phpversion(), '5.4', '<') ) {
	die ( "required PHP version 5.4 or greater. You need upgrade PHP version on your server." );
}
...
abstract class DLEPlugins {
	...
	private static function loadplugins() {
		...
		$db->query( "SELECT id, needplugin FROM " . PREFIX . "_plugins ORDER BY id ASC", false );
